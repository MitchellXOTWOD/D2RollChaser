...
const Home = () => {
  ...
  return (
    <div className="page-container h-full md:flex bg-black">
      <Sidebar setSelectedItem={setSelectedItem} openSidebar={openSidebar} setOpenSidebar={setOpenSidebar}/>
      {isWeaponSelected && selectedWeapon && (isSmallScreen ? !openSidebar : true) && (
        <div className="weapon-container pb-5 flex flex-col md:h-auto w-full space-y-7 pt-5 md:px-36 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-gray-400">
          <h1 className="font-medium text-white">{selectedWeapon.displayProperties.name}</h1>
          <div className="weapon-ss self-center md:w-1/2 w-full">
            <Image src={`${bungieAPI}${selectedWeapon.screenshot}`} height={1080} width={1920} alt="weapon-screenshot" unoptimized/>
          </div>
          ...
          <div className="flex items-center justify-center space-x-5">
            ...
            {odds && <p className="text-white">rolled {refreshes} time(s) with a {odds.toFixed(3)}% chance</p>}
          </div>
        </div>
      )}
    </div>
